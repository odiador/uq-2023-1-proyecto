...
public class PanelAgregarCamioneta extends PanelConVolver {
	private Combustible combustible;
	private EventHandler<? super MouseEvent> eventoVolver;

	public PanelAgregarCamioneta(EventHandler<? super MouseEvent> eventoVolver) {
		initComponents();
	}
	...
	public void initComponents() {
		...
		box.getChildren().add(generarHBox("Ingrese la capacidad del maletero", tfCMaletero));
		...
		Boton botonAgregar = new Boton("Aceptar", e -> {
			ControlVehiculos.agregarCamioneta(tfPlaca.getText().trim().toUpperCase(), tfMarca.getText().trim(),
					tfModelo.getText().trim(), tfCilindraje.getText().trim(), tfVelocidadMax.getText().trim(),
					combustible, cbEstadoVechiculo.getValue(), cbTipoCambio.getValue(),
					tfNPasajeros.getValue().toString(), tfNBolsasAire.getValue().toString(),
					tfNPuertas.getValue().toString(), cbTieneAire.isSelected(), cbTieneCamara.isSelected(),
					cbTieneABS.isSelected(), tfCMaletero.getText().trim(), cbTieneCrucero.isSelected(),
					cbTieneColision.isSelected(), cbTieneTrafico.isSelected(), cbTieneAsistente.isSelected(),
					cbEs4x4.isSelected());
		});
